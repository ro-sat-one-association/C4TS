...
while True:
  if imu.IMURead():
    ...
    f.write("r: %f p: %f y: %f" % (math.degrees(fusionPose[0]), 
        math.degrees(fusionPose[1]), math.degrees(fusionPose[2])) + "\n")
        
        
    if (data["pressureValid"]):
        print("Pressure: %f, height above sea level: %f" % (data["pressure"], computeHeight(data["pressure"])))
        f.write("press: %f, altit: %f" % (data["pressure"], computeHeight(data["pressure"])) + "\n")
    if (data["temperatureValid"]):
        print("Temperature: %f" % (data["temperature"]))
        f.write("temp: %f" % (data["temperature"]) + "\n\n")
    f.close()
